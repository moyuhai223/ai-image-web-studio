...
### 修复(Studio Console V1 视觉打磨)

- **修复 v0.8.13 创作页「界面混乱」**:上版三处视觉问题一并修掉——
  - **画布不再是巨大深色空洞**:放弃「整页锁高(100dvh)」的激进应用式布局,改回自然文档流(底部 Composer 仍 `sticky` 贴视口底,内容多时随页面滚)。空态画布不再被强行撑满整页,给合理最小高度并居中提示。
  - **去掉画布空态的装饰性大色块**:空态占位块(`.preview-placeholder`)在深色画布上显示成两个突兀深块,现于创作台画布隐藏,只保留居中的简洁提示。
  - **画布背景不再超沉**:移除 `--canvas-bg` 深底,画布与其它面板一致的中性底,消除大面积深色块堆叠。
  - **右栏参考图恢复默认折叠**:上版把参考图区改成右栏恒展开(6 张缩略图占满、右栏过载),现恢复「默认收起、点击展开、选了参考图自动展开」的原交互,右栏清爽。
