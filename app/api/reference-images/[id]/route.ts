...
import { getReferenceImageById } from "@/lib/repository";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!image || (user.role !== "admin" && image.user_id !== user.id)) {
  ...
  const etag = thumbnail
    ? `W/"thumb-ref-${image.id}-${stats.size}-${Math.round(stats.mtimeMs)}"`
    : `"${image.checksum}"`;
